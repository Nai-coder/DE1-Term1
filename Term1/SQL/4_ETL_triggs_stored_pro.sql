...
DROP PROCEDURE IF EXISTS Get2010DataByCountry;

DELIMITER //

CREATE PROCEDURE Get2010DataByCountry(
...
CALL Get2010DataByCountry('Hungary');
CALL Get2010DataByCountry('Austria');
-- France is the highest nuclear el. producer in Europe:
CALL Get2010DataByCountry('France');
...
DROP EVENT IF EXISTS CreateDataCheckSwitzerland;

DELIMITER $$

CREATE EVENT CreateDataCheckSwitzerland
...
-- we note that the second table wdi_2010 is also updated when wdi_pop gets a row inserted:
SELECT * FROM wdi_2000 WHERE `Country Name` = 'Fantasia';
